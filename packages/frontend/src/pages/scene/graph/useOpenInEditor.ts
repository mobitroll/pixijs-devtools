      ...
      if (!source) {
        ...
        const message = hasTaggedClasses
          ...
          : `[PixiJS DevTools] "Open in editor" needs source locations, but none were found on the page. ` +
            `Enable the \`pixiDevtoolsSource()\` plugin from \`@kahoot/pixi-devtools/vite\` in your app's Vite dev config.`;

        logToPage(hasTaggedClasses ? 'warn' : 'error', message);
        return;
      }

      // The editor is injected into the page by the @kahoot/pixi-devtools/vite plugin; default to VSCode.
